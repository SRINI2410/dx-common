...
/**
 * Generic builder for constructing {@link PaginatedRequest} from HTTP query parameters.
 *
 * <p>Supports configurable filter mapping, temporal queries, sorting, and pagination.
 * Used across all DX microservices for standardized paginated list endpoints.
 *
 * <h3>Usage:</h3>
 * <pre>{@code
 * PaginatedRequest request = PaginationRequestBuilder.from(ctx)
 *     .allowedFiltersDbMap(Map.of("status", "status_column", "type", "type_column"))
 *     .allowedSortFields(Set.of("createdAt", "name"))
 *     .apiToDbMap(Map.of("createdAt", "created_at", "name", "name"))
