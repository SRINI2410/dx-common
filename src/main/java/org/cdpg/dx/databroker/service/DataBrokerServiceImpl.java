...
public class DataBrokerServiceImpl implements DataBrokerService {
  ...
  public Future<Void> deleteQueue(String queueName, Vhosts vhosts) {
    // Delegate to user-specific version with null userid
    return deleteQueueForUser(queueName, null, vhosts);
  }

  @Override
  public Future<Void> deleteQueueForUser(String queueName, String userid, Vhosts vhosts) {
  ...
  public Future<JsonObject> executeAdapterQueryRPC(JsonObject request) {
    ...
    rabbitClient
        ...
        .onSuccess(
            publishSuccess -> {
              LOGGER.debug("executeAdapterQueryRPC success");
              promise.complete(publishSuccess);
            })
        .onFailure(
            publishFailure -> {
              LOGGER.error("executeAdapterQueryRPC failure");
              promise.fail(publishFailure);
            });
    return promise.future();
  }
